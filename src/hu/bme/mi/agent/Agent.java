...
public class Agent implements AgentsTurnListener{
	 ...
	 * @param board	A közös valóságalap a tábla aktuális állapota
	 * @param color	Az MI játékos színe
	 */
	...
	private void nextMove(){
		...
		ArrayList<Cell> possibleAttackCells = actual.getFigurePossibleAttack(color);
		if(possibleAttackCells.size() > 0){
			//Van kötelezõ ütés
			for (Cell cell : possibleAttackCells) {
				ArrayList<Cell> attackTargetCell = actual.getCellPossibleAttack(cell);
				for (Cell cell2 : attackTargetCell) {
					possibleNextMoves.add(new Movement(cell, cell2, attackHeuristica(cell, cell2)));
				}
			}
		} else {
			//Szabad lépés
			ArrayList<Cell> possibleMoveCells = actual.getFigurePossibleMove(color);
			for (Cell cell : possibleMoveCells) {
				ArrayList<Cell> moveTargetCell = actual.getCellPossibleMove(cell);
				for (Cell cell2 : moveTargetCell) {
					possibleNextMoves.add(new Movement(cell, cell2, idleHeuristica(cell, cell2)));
				}
			}
		}
		try {
